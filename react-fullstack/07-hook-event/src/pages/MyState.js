...
const MyState = () => {
   ...
   * - state값(상태값)은 직접 변경할 수 없고 반드시 setter를 통해서만 변경 가능하다.
   * - useState() 함수에 전달하는 값은 state값에 대한 초기값이다.
   */
                
  const [myName, setMyName] = React.useState('');   //const myName = '';
  const [myPoint, setMyPoint] = React.useState(50); // const myPoint = 50;
    ...
    <div>
      ...
      <div>
        <label htmlFor="myNameInput">이름: </label>
                {/**input타입에 value값에도 상태값(myName)을 같이 건다. change를 걸고 함수호출*/}           
