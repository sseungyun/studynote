...
const Department = () => {
    ...
    /** form 에서 submit이벤트가 발생할 때 호출된 이벤트 핸들러. */
    // 성능 최적화를 위해 useCallback() 적용함
    const onDepartmentSave = React.useCallback((e) => {
        // 페이지 강제 이동을 차단
        e.preventDefault();
        
        //<form>안에 있는 입력 요소의 값 추출 </form>
        ...
        setTimeout(() => {
            (async () => {
                ...
                try {
                    ...
                    const response = await axios.post('httml://localhost:3001/department', {
                        'dname': dname,
                        'loc': loc
                    });
                ...
                if (json != null) {
                    ...
                    // --> 상태값을 파라미터로 받는 콜백에서 상태값 상태로 갱신 결과를 리턴
    ...
    React.useEffect(() => {
        if(dropId > -1) {
            ...
            // 성능향상을 위해 상태값을 함수형 업데이트로 처리함
            // --> 상태값을 파라미터로 받는 콜백에서 상태값 갱신결과를 리턴
    ...
    const onUpdateSubmit = React.useCallback(e => {
        ...
        // 이벤트가 발생한 <From>요소 취득</From>
        const current = e.target;

        // <From>요소 내의 <input> 요소 들을 name 속성값으로 접근하여 입력값을 얻음</From>
        ...
        (async () => {
            ...
            if (json !== null) {
                ...
                setDepartment(department => {
                    ...
                    // 원본 생태값의 해당 인덱스 번호 (find) 위치부터 1개를 ajax로 반환받은 수정결과 데이터로 교체한다.
