...
import { myip } from "../helper/UtilHelper";
...
// listen을 시작 --> backend(sever)가 가동을 시작했다는 의미.
server.listen(port, () => {
...
server.on('request', (req, res) => {
    ...
    if(req.url == '/sample.html') {
        ...
        const path = __dirname + '/07-HTTPServer/crud-axios-sample.html';
        fs.createWriteStream(path).pipe(res);
    } else {
        // 클라이언트에게 전송할 응답 헤더 구성
        res.writeHead(200, {
            // 브라우저에게 인식시킬 출력 내용의 컨텐츠 형식
            ...
            /** CORS 접근 허용을 위한 설정 */
            // 접근을 허용할 도메인 혹인 IP(브라우저에 출력되고 있는 도메인을 의미함, *은 ALL의의미)
...
// 예제이브로 타이머를 통해 백엔드를 60초 후 강제 종료
setTimeout(() => {
      server.close();
 }, 60000);  
